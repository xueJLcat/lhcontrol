...
const zhCN = {
  ...
  'a recent successful scan is required before changing a channel': '更改频道前需要先完成一次有效扫描',
  'a recent successful scan is required: one or more visible stations have an unknown channel': '需要先完成一次有效扫描：一个或多个可见基站的频道未知',
  'station is transitioning between power states': '基站正在电源状态切换中',
  'power write is unavailable': '电源写入不可用',
  ...
  'power.error.bootingRetry': '基站正在启动中，请待状态切换完成后重试：{detail}',
  'channel.error.bootingRetry': '基站正在启动中，请待状态切换完成后重试频道修改：{detail}',
  'station.error.notFoundDetail': '未找到基站：{detail}',
  'station.error.notSeenInLatestScan': '基站 {address} 未在最近一次扫描中发现',
  'channel.error.conflictDetail': '频道与另一个可见基站冲突：频道 {channel} 已被 {name}（{address}）使用',
  'error.operationUnsupported': '不支持该操作',
  'error.operationUnsupportedDetail': '不支持该操作：{detail}',
  ...
  'status.readIncomplete': '基站状态读取不完整：{detail}',
  'scan.initialReadIncomplete': '初始读取不完整：{detail}',
  'status.refreshIncomplete': '一个或多个基站的状态读取未完成：{detail}',
  'status.readSkipped': '状态读取被跳过：{detail}',
  'status.readCancelled': '状态读取被取消：{detail}',
  'status.readDeadlineExceeded': '状态读取超时：{detail}',
  'status.refreshDeadlineExceeded': '状态刷新超时：{detail}',
  'bluetooth.unavailable': '蓝牙不可用；请开启蓝牙或检查适配器后重试：{detail}',
  'bulk.cancelTimeout': '批量电源操作在取消后 {duration} 内未停止',
  ...
  'settings.error.mustNotFallBelow': '{subject}不得低于{other}（{limit} 秒），当前为 {value}',
