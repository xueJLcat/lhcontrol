...
func (u *UUID) UnmarshalText(s []byte) error {
	// The decode paths accumulate bits with |=, so a receiver that already
	// holds a value (a reused variable, an encoding/json overwrite) would
	// silently OR old and new bits together. Reset before decoding, matching
	// MAC.UnmarshalText.
	*u = UUID{}
	switch len(s) {
	case 36:
