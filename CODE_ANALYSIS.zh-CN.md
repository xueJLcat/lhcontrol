...
- 生产版始终写入有大小上限的诊断日志：
  `%APPDATA%\lhcontrol\lhcontrol.log`，并保留一个 `lhcontrol.log.1` 备份段。

## 同步修正的功能问题

- 同一基站的重复请求立即返回 Busy，不再排队阻塞下一次扫描。
- 独立设备 GATT 操作最多并行 2 个；扫描、全量状态读取和批量电源操作保持互斥。
- 批量电源控制覆盖本次应用会话内所有已发现设备；设备即使因停止广播而被最新扫描漏掉，也不会导致 All 控件永久禁用。
...
项目当前正式限定为 Windows 10/11 x64。仓库中的 TinyGo Bluetooth 本地模块只保留
Windows 源码，并非可用于 Linux 或 macOS 的完整上游模块。
...
- WinRT 扫描停止错误已映射为无线电不可用、资源占用和策略禁用，不再统一显示为停止扫描失败。
- 启动时蓝牙不可用不再永久锁死当前进程；开启蓝牙后等待短暂退避即可重新扫描。
- Manager 关闭后拒绝新操作，状态轮询遇到 Busy 设备会跳过而不是循环等待。
- 前端使用统一操作状态和异步 revision，旧请求不能覆盖更新的扫描或控制结果。
- Vitest 覆盖 All 控件可用性、操作锁、过期异步结果和销毁行为。
- HTTP 路由可通过 fake manager 做集成测试，并限制请求体和连接超时。
- 单实例检测使用 Windows named mutex，不再占用 TCP 端口。
- 发布构建固定 Go 1.25.12、Node 22.14、Wails 2.13.0 和 NSIS 3.12，并校验版本、产物和 SHA-256。
